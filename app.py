...
st.set_page_config(
    page_title="Ragster", page_icon="assets/ragster_logo.png", layout="wide"
)
...
if os.path.exists(logo_path):
    ...
    st.sidebar.markdown(
        f"""
    <div class="st-title">
        <img src="data:image/png;base64,{encoded_image}" alt="Ragster Logo" class="logo" style="width: 900px; height: 300px; margin-bottom: 10px;">
    </div>
    """,
        unsafe_allow_html=True,
    )
...
input_lang = st.sidebar.selectbox(
    ...
    options=list(language.LANGUAGES.values()),
    index=0,
    help="Choose 'auto' to auto-detect user input language"
)
...
output_lang = st.sidebar.selectbox(
    ...
    options=list(language.LANGUAGES.values()),
    index=1,
    help="Language in which you want AI responses"
)

# Get language code from name
input_lang = [k for k, v in language.LANGUAGES.items() if v == input_lang][0]
output_lang = [k for k, v in language.LANGUAGES.items() if v == output_lang][0]

# Update Ragster instance with selected languages
law.set_languages(input_lang, output_lang)
